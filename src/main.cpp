// Copyright (c) 2009-2010 Bartcoin Nakamoto
// Copyright (c) 2009-2016 The Bartcoin developers
...
const string strMessageMagic = "BartCoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, const CBlockIndex* pindex)
{
     ...
     if (nPoWHeight == 1)
       nSubsidy = 400000000 * COIN; //Premin: 400M
     else
       nSubsidy = 0 * COIN;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal BartCoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in BartCoin
    bool fStrictPayToScriptHash = true; // Always active in BartCoin
    ...
    // ppcoin: fees are not collected by miners as in BartCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BartCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Oct 14, 2016. The BartCoin release and hope that it is a great coin.";
        CTransaction txNew;
      ...
      if (true  && (block.GetHash() != hashGenesisBlock))
        ...
        assert(block.hashMerkleRoot == uint256("0x33aaa2a39fa07889c4308f6991a01b432c2499f509c98ec59cf9d60465e98e15"));
...
unsigned char pchMessageStart[4] = { 0x10, 0x14, 0x15, 0x53 };
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    // 1-BartCoin-fee transactions. It should be set above the real
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("BartCoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("BartCoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BartCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("BartCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadBartCoinMiner(void* parg);

static bool fGenerateBartCoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void BartCoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("BartCoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateBartCoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        ...
        while (vNodes.empty() || IsInitialBlockDownload() || pwallet->IsLocked())
        {
            ...
            if (fShutdown)
                return;
            if (!fGenerateBartCoins && !fProofOfStake)
                return;
        }
        ...
        printf("Running BartCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBartCoinMiner(void* parg)
{
    ...
    try
    {
        ...
        BartCoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadBartCoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadBartCoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadBartCoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateBartCoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateBartCoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateBartCoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d BartCoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadBartCoinMiner, pwallet))
                printf("Error: NewThread(ThreadBartCoinMiner) failed\n");
            Sleep(10);
        }
    }
}
